...
if (!SUPABASE_URL || !SUPABASE_KEY || !GOOGLE_PLACES_API_KEY) {
  console.error(
    "❌ .env.local에 VITE_SUPABASE_URL / VITE_SUPABASE_PUBLISHABLE_KEY / GOOGLE_PLACES_API_KEY 를 채워주세요.",
  );
  process.exit(1);
}
...
console.log("✅ 환경변수 로드 + Supabase 클라이언트 생성 완료");
...
async function main() {
  ...
  const { data: restaurants, error } = await supabase
    .from(TABLE_NAME)
    .select(`${ID_COLUMN}, ${NAME_COLUMN}, ${ADDRESS_COLUMN}, ${IMAGE_COLUMN}`)
